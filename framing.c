...
printf("length: %i\n",length);
while(above288){ //loops until length is less than 32
    if(length>290){
            memset(copystr,0,sizeof(copystr)); //resets copystr
            strcat(frame,startbit); //adds start bit
            memcpy(copystr,&data[dataloc],288); //copies 32 bits of data
...
while(above288){ //loops until length is less than 32
    if(length>34){
            ...
            strcat(frame,stopbit); //adds stop bit
            strcat(frame,delimiter);
            dataloc+=34; //shifts the index of the data to be copied
            length-=34; //reduces length for if comparison
           // printf("copystr: %s \n",copystr);
    }
    else {
            memset(copystr,0,sizeof(copystr));
